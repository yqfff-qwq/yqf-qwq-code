...
int pkint (ifstream &output, int l, int r)  {
        int x;
        output >> x;
...
int main(int argc, const char** argv){
    ...
    */
    int ix = pkint(output, x, 1000);
    for (int c = 1; c <= ix; ++c){
    	int ksize=0;
        vector<int> k(ksize=pkint(output, 1, n));
        output.get();
        for (int i=0;i<ksize;i++)k[i] = pkint(output, 1, n);
        if (set<int>(k.begin(), k.end()).size() < k.size())WA();
        int l = k.size();
        vector<int> v;
        for (int i = 0; i < l; ++i)v.push_back(p[k[i]]);
