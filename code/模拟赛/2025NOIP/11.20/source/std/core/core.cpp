...
struct point{
    int x,y,v;
}ps[N+1],pt[N<<1|1],rs[N+1],rt[N<<1|1];
struct Map{
    uint mod=29947769;
    ull base;
    Mod M;
    ...
    int tot;
    int head[LIM];
    void insert(cint x,cint y,cint id)
    {
        cint v=M(1ll*base*x+y);
        L[++tot]={x,y,id,head[v]};
        head[v]=tot;
    }
    int ask(cint x,cint y)
    {
        cint v=M(1ll*base*x+y);
        for(int i=head[v];i;i=L[i].nxt)
        {
            if(L[i].x==x&&L[i].y==y)return L[i].id;
        }
        return 0;
    }
}mps,mpt;
...
int main()
{
    ...
    for(int i=1;i<=n;++i)
    {
        cint p=mps.ask(ps[i].x,ps[i].y);
        if(!p)
        {
            rs[++cs]=ps[i];
            mps.insert(ps[i].x,ps[i].y,cs);
        }
        else
        {
            ++rs[p].v;
    ...
    for(int i=1;i<=m;++i)
    {
        cint p=mpt.ask(pt[i].x,pt[i].y);
        if(!p)
        {
            rt[++ct]=pt[i];
            mpt.insert(pt[i].x,pt[i].y,ct);
        }
        {
            (rt[p].v+=pt[i].v)>=MOD?(rt[p].v-=MOD):0;
        }
    }
    for(int i=1;i<=cs;++i)
    {
        for(int j=1;j<=ct;++j)
        {
            ans=(ans+1ll*calc(rs[i].x,rs[i].y,rt[j].x,rt[j].y)*rs[i].v%MOD*rt[j].v)%MOD;
        }
    }
    print(ans);
    return 0;
