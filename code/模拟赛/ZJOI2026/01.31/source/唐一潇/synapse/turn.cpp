...
inline void Local_File1(string File_Name, int File_Idx) {
    freopen((File_Name + to_string(File_Idx) + Anssuffix).c_str(), "r", stdin);
    freopen((File_Name + to_string(File_Idx) + ".txt").c_str(), "w", stdout);
    return;
}
} // namespace FILE_IO
...
signed main() {
    Init_IO();
    int id = 1, T, x;
    Local_File("synapse", id);
    cin >> T;
    fclose(stdin);
    fclose(stdout);
    Local_File1("synapse", id);
    for (; T--;) {
        bool flag = false;
        cout << "0 ";
        for (; cin >> x;) {
