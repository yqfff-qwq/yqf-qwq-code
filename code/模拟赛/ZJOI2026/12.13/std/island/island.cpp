...
int main() {
  ...
  while (cin >> A >> B >> C >> X >> Y) {
    ...
    function<void(int64_t,int64_t,int64_t,int64_t,int64_t,int64_t)> rec = [&](int64_t depth, int64_t a, int64_t b, int64_t c, int64_t x, int64_t y) {
      ...
      if (a == A && b == B && c == C) {
        ++cnt;
        int64_t tmp = depth + 2*solve(xc[a+b] ? a+b : -1, xc[a+c] ? a+c : -1, xc[b+c] ? b+c : -1, x)
                        + 2*solve(yc[a+b] ? a+b : -1, yc[a+c] ? a+c : -1, yc[b+c] ? b+c : -1, y);
        ret = min(ret, tmp);
        // if (tmp == 8) cout << depth << ' ' << xc[a+b] << ' ' << xc[a+c] << ' ' << xc[b+c] << ' ' << yc[a+b] << ' ' << yc[a+c] << ' ' << yc[b+c] << endl;
      }
